...
> 状态：用户已批准；2026-08-13 补充实时任务 cutoff 锁定语义
>
> 日期：2026-08-13
>
> 市场范围：A 股行业板块与概念板块
...
1. 创建 `AnalysisRun`，锁定任务 ID、交易日、时区、运行模式和 `requested_at`。
2. 对实时任务并行获取核心行情；对历史回放任务使用启动时给定的 `requested_cutoff_at`。
3. 实时任务完成核心行情质量检查后，根据实际市场观测时间锁定不可变的 `run_cutoff_at`；若不同来源时点偏差超限则失败或降级。
4. 获取或过滤新闻、成分股和可选热度，确保进入分析的事实都不晚于已锁定 cutoff。
5. 标准化数据并执行覆盖率、新鲜度、时点一致性和异常值检查。
6. 对行业与概念板块分别计算市场事实与“值得写”指标。
7. 通过多路召回得到 8～12 个候选，并对高度重叠概念做聚类去重。
8. 对候选板块并行执行证据匹配和谨慎归因。
9. 精选 3～6 个板块，构造文章主线和篇幅分配。
10. 生成通用社区稿。
11. 执行事实、时间、引用、归因和合规核验；只返工有问题的句段。
12. 将草稿与证据卡送入审核台。
...
- `AnalysisRun`：一次独立运行；包含交易日、模式、`requested_at`、可选 `requested_cutoff_at`、锁定后的 `run_cutoff_at`、`cutoff_locked_at`、状态和总体预算。
...
- `cutoff_locked_at`：系统把本轮 cutoff 持久化为不可变值的时间。

`published_at` 或 `observed_at` 晚于 `run_cutoff_at` 的内容不得进入本轮归因。缺失发布时间的新闻只能作为背景，不能支持因果。

实时模式与历史模式的 cutoff 规则不同：

- `LIVE`：任务启动先记录 `requested_at`；核心行情采集和质量检查完成后，以实际 Provider 观测时间锁定 `run_cutoff_at`。锁定后不得再向前移动，后续新闻和热度只保留不晚于该时点的数据。
- `AS_OF`：启动时必须给出 `requested_cutoff_at`，并立即作为 `run_cutoff_at` 锁定；只允许使用支持历史时点查询的 Provider。
- 多个核心行情来源的 `observed_at` 偏差超过配置窗口时，不得假装成同一时点快照；必须降级为单一可信来源或使该轮失败。
...
- 盘中任务不得读取盘后新闻；
- 同一自然日多次任务互不污染；
- LIVE 任务只有在核心行情完成后锁定 cutoff，锁定后不能移动；
- AS_OF 任务在启动时锁定指定 cutoff，实时 Provider 不得冒充历史查询能力；
- 发布时间缺失的新闻不能升级为因果证据；
- 时区和交易日边界正确；
- 历史回放不得发生前视偏差。
