...
升级后的生产浏览器测试通过，但额外开发模式检查发现已有的请求生命周期缺陷：StrictMode 的 effect 清理取消请求后，总览仍复用该请求，导致首次数据不再加载；工作台的旧请求收尾也可能清除新请求的进行中标记。后续开发浏览器回归还发现审核队列过早结束加载，以及自动保存的 mounted 标记清理后未恢复，导致输入与保存状态不更新。保留 StrictMode，限定修改这四个 hook 的请求身份校验与清理，不改页面布局、业务 API 或轮询间隔。先新增 StrictMode 重挂载、旧请求晚结束及自动保存状态的失败测试，再实现修复。

增加可重复的开发模式浏览器验收。只有显式开发测试模式允许 GET 请求的 `net::ERR_ABORTED`（已实际观测到 StrictMode 主动取消）；所有断网失败、HTTP 错误、未处理 API、页面异常仍检查，生产测试保持原严格规则。开发、生产模式均执行完整 40 项行为断言，不能用取消过滤掩盖总览空白。
