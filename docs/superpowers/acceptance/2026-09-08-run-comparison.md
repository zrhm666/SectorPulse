...
运行对比三阶段功能先在隔离工作区完成本地验收；随后用户要求继续，已于同日合入本地 main 并重建主目录 `web/dist`，详见文末“本地集成”。本结论只针对本功能与本轮回归范围，不代表全部增强路线图或真实上游服务已经验收。
...
- 功能验收时尚未合并 main，后续本地集成见下节；未推送远端、未验证远端 CI。

## 本地集成

2026-09-08，用户在“下一步本地合并并重建主目录前端”的交付说明后要求继续。核对 main 与功能工作区均无未提交改动，合并基点为 `5099334`；执行 `git merge --ff-only codex/run-comparison`，main 快进到功能提交 `19fe775`。未拉取或推送远端，不引入未经核验的远端变动。

合并前重新验证功能分支：前端 211 项、后端 400 项通过。合并后从 `D:/work/SectorPulse` 及其 `web` 目录执行：

| 检查 | 实际结果 |
| --- | --- |
| `npm.cmd run build` | TypeScript/Vite 成功；资源版本与功能分支一致 |
| `npm.cmd test -- --maxWorkers=2` | 58 文件、211 项通过，60.16 秒 |
| 后端普通回归，同上文 pytest 参数，PYTHONPATH 指向主目录 | 400 passed、14 skipped、24 deselected、1 warning，69.17 秒 |
| `npm.cmd run test:e2e` | 50 项通过，16.7 秒 |

以上命令退出码均为 0。主目录 CSS 和 JS 均与功能分支构建逐文件 SHA-256 相同：

- `index-C168tcnG.js`：`BF08706F2DFB93A8588F3234A4325BAEBD4994E8B51053CF783CCF52074E3F2B`
- `index-DxyxkyQW.css`：`8A597D85EC4C4456334B2BBBB50BD0FD67BE0EE6A15C0D1F277566C799AFC833`

依赖锁文件未变化，使用主目录现有依赖完成构建和验证。此次集成不重新启动专用 PostgreSQL、不重复执行其专项或开发浏览器套件，相关证据来自同一产品代码的前述验收。没有启动业务服务、触发调度或修改业务库，未改 `.env` 和 consent；开始检查时 8000/9000/4173 均无监听服务。

运行入口仍为主目录 `.venv` 的 `python -m sector_pulse.web.server`，默认地址 `http://127.0.0.1:9000`。启动后从“分析运行”进入“运行对比”，也可访问 `/runs/compare`。

保留功能分支和 `.worktrees/run-comparison`，其中含被 Git 忽略的临时 PostgreSQL 实例与验收材料；未执行强制删除。既有其他工作区未改动。本节及项目进度作为单独的本地文档提交保存，不影响已经验证的产品代码。
