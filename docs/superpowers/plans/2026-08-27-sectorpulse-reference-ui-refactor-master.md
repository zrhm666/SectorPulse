...
- [x] 桌面使用 22%/53%/25% 三栏工作区并保证中栏阅读宽度。
- [x] 草稿视觉连续但继续保存结构化字段、版本、哈希和证据链接。
- [x] 自动保存覆盖未保存、保存中、成功、失败和版本冲突。
- [x] 来源随当前字段或段落变化，证据决策支持保留、降权、排除和理由。
- [x] 批准、退回和撤销严格服从后端可执行操作与审计。
- [x] 窄屏使用“队列/草稿/证据”页签，不压缩三栏。
- [x] 通过并发版本冲突、证据判定和审批回归测试。

Phase 4 accepted on 2026-08-28; see `docs/superpowers/acceptance/2026-08-28-sectorpulse-reference-ui-phase4.md`.
