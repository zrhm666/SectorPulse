...
| Phase 1 | 分层设计基础、统一外壳、路由上下文、响应式导航 | Phase 0 | 全部现有路由可达，桌面/移动外壳验收通过 |
...
- [ ] 建立新 CSS 分层入口并将现有 `styles.css` 作为迁移期 `legacy` 层保留。
- [ ] 建立所有已知路由的页面上下文解析，不增加死链接。
- [ ] 高保真重构统一侧边栏、品牌区、导航高亮、顶部上下文和独立主滚动区。
- [ ] 增加统一按钮与页面标题元信息契约，保持现有组件调用兼容。
- [ ] 通过 Vitest、TypeScript/Vite build、Playwright 桌面和抽屉导航检查。
- [ ] 使用浏览器工具在 1536×1024、1024×768、390×844 下完成视觉验收并记录报告。
