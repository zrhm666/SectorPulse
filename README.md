...
数据采集完成并确认候选板块后，可在“生成分析稿”前选择归因方式：

| 模式 | 行为 | 适用场景 |
| --- | --- | --- |
| 工作流模式（默认） | 使用已采集证据执行既有归因流程 | 日常分析，减少额外模型调用 |
| Agent 模式（实验性） | 模型自主决定检索新闻、阅读详情、核对锁定行情或结束查证 | 需要追加查证，接受更多耗时和 Token 消耗 |

两种模式共用后续编辑、写作、自动审核和人工批准流程。运行列表、详情可查看模式；Agent 详情提供按板块保存的查证记录。重试继承模式并创建新运行，不是从中断的模型轮次继续。

Agent 每板块默认最多 6 轮决策、5 次工具调用、180 秒，同时处理最多 2 个板块。`config/llm.yaml` 可配置 `agent_limits`、`max_agent_calls`（默认 80）和 `max_agent_tokens`（默认 500000）；共享额度覆盖后续写作。一次逻辑模型调用可能由适配器重试至多 3 次，因此 80 次并不等于 80 次 HTTP 请求。预算采用保守预留，可能提前停止；模型价格未配置时显示“费用未知”，金额上限无法可靠计算，但调用数与 Token 限制仍生效。

真实模式继续要求已有的数据及 LLM 授权配置。正文只读取受支持的公开来源；读取失败明确降级为摘要。后来获取的网页仅作为补充背景，不自动升级为历史因果证据。Fixture 是合成演练，不代表真实检索成功。

当前已完成同输入离线双模式对比、双方言存储和界面验证；真实模型已在隔离环境完成“检索新闻 → 核对演练行情 → 结束归因”。尚未完成真实模型原文阅读分支及真实同输入双模式质量对比。该抽测不代表真实行情到最终草稿的完整验收。详见[实施与验收记录](docs/superpowers/plans/2026-09-10-attribution-agent-modes.md)。
...
3. 生成分析稿时选择归因方式：首次使用建议 **工作流模式**；需要追加查证时再选 **Agent 模式**。
...
回到“系统状态”确认数据库、数据源、模型和授权均为就绪，再创建 Live 运行。Live 运行会访问外部数据源并消耗模型额度；Agent 模式还可能进行额外新闻检索和详情读取。
