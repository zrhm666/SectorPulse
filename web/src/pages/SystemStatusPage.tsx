...
export default function SystemStatusPage() {
  ...
  return <section>
    ...
    {!loading && !error && summary && <div className="status-grid">
      <Panel title="数据库"><StatusBadge status="READY" label={summary.database.backend === 'postgresql' ? 'PostgreSQL 已连接' : 'SQLite 已连接'} /><p className="status-detail">数据库：{summary.database.name}</p></Panel>
      <Panel title="LLM"><StatusBadge status={summary.llm.configured ? 'READY' : 'UNAVAILABLE'} label={summary.llm.configured ? '配置已就绪' : '配置不完整'} /><p className="status-detail">提供方：{summary.llm.provider}<br />模型：{summary.llm.model || '未设置'}<br />单次预算：¥{summary.llm.budget_cny_per_run}</p></Panel>
      <Panel title="运行授权"><StatusBadge status={summary.providers.live_data_available ? 'READY' : 'UNAVAILABLE'} label={summary.providers.live_data_available ? '实时数据已就绪' : '实时数据不可用'} />{summary.providers.missing_requirements.map((item) => <p className="status-detail" key={item}>缺少 {item}</p>)}<p className="status-detail">实时 LLM 授权：{summary.consent.live_llm ? '已确认' : '未确认'}</p></Panel>
    </div>}
  </section>
}
