...
async def run_phase0_probe(
    ...
    iq, cq = evaluate_universe(industry, thresholds), evaluate_universe(concept, thresholds)
    ...
    report = report.model_copy(
        update={
            "run": lock_cutoff_from_core_market(
                run, [industry, concept], datetime.now(UTC), max_skew_seconds
            )
        }
    )
    write_utf8_atomic(output_dir / "industry.json", industry.data.model_dump_json(indent=2))
    write_utf8_atomic(output_dir / "concept.json", concept.data.model_dump_json(indent=2))
    radar = {
        "industry": [r.model_dump(mode="json") for r in build_diagnostic_radar(industry.data)[:20]],
        "concept": [r.model_dump(mode="json") for r in build_diagnostic_radar(concept.data)[:20]],
    }
    write_utf8_atomic(output_dir / "radar.json", json.dumps(radar, ensure_ascii=False, indent=2))
    write_utf8_atomic(output_dir / "probe.json", report.model_dump_json(indent=2))
    return report
