...
class CutoffAlreadyLockedError(RuntimeError): pass
class InvalidCutoffError(ValueError): pass
...
class AnalysisRun(BaseModel):
    ...
    def validate_state(self):
        if self.mode is AnalysisMode.AS_OF and (self.requested_cutoff_at is None or self.run_cutoff_at != self.requested_cutoff_at or self.cutoff_locked_at is None):
    ...
    def create_as_of(cls, requested_at: datetime, requested_cutoff_at: datetime):
        if requested_cutoff_at > requested_at: raise InvalidCutoffError("cutoff cannot be later")
        return cls(run_id=uuid4(), mode=AnalysisMode.AS_OF, requested_at=requested_at, requested_cutoff_at=requested_cutoff_at, run_cutoff_at=requested_cutoff_at, cutoff_locked_at=requested_at)

    def lock_live_cutoff(self, observed_at: datetime, locked_at: datetime):
        if self.mode is not AnalysisMode.LIVE: raise InvalidCutoffError("only LIVE runs")
        if self.run_cutoff_at is not None: raise CutoffAlreadyLockedError("already locked")
        if observed_at > locked_at: raise InvalidCutoffError("observed_at cannot be later")
