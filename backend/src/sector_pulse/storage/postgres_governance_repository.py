...
class PostgresGovernanceRepository:
    ...
    async def save_evidence_decision(self, decision: EvidenceDecision) -> None:
    ...
    async def list_evidence_decisions(self, draft_id: UUID) -> tuple[EvidenceDecision, ...]:
    ...
    async def save_preference_candidate(self, candidate: PreferenceCandidate) -> None:
        async with self._database.engine.begin() as connection:
            await connection.execute(
    ...
    async def adopt_preference(self, candidate_id: UUID, adopted_at: datetime) -> PreferenceVersion:
        async with self._database.engine.begin() as connection:
            result = await connection.execute(
            ...
            result = await connection.execute(text("SELECT COALESCE(MAX(version), 0) FROM preference_versions"))
            version = int(result.scalar_one()) + 1
            await connection.execute(text("UPDATE preference_versions SET active = 0 WHERE active = 1"))
            await connection.execute(
            ...
            await connection.execute(text("UPDATE preference_candidates SET status = 'ADOPTED' WHERE candidate_id = :candidate_id"),
                                     {"candidate_id": str(candidate_id)})
