...
class SQLitePhase1BRunsRepository:
    ...
    def _row_to_model(row: object) -> Phase1BRunRow:
        ...
        return Phase1BRunRow(
            run_id=UUID(r[0]),  # type: ignore[arg-type]
            requested_at=datetime.fromisoformat(r[1]),  # type: ignore[arg-type]
            provider=r[2],  # type: ignore[arg-type]
            status=r[3],  # type: ignore[arg-type]
            elapsed_ms=r[4],  # type: ignore[arg-type]
            total_cost_cny=r[5],  # type: ignore[arg-type]
            input_json_hash=r[6],  # type: ignore[arg-type]
            draft_id=UUID(r[7]) if r[7] else None,  # type: ignore[arg-type]
            error_message=r[8],  # type: ignore[arg-type]
            finished_at=datetime.fromisoformat(r[9]) if r[9] else None,  # type: ignore[arg-type]
        )
